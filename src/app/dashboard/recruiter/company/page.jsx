...
export default function RecruiterCompanyPage() {
  ...
  const [companies, setCompanies] = useState([]); // একাধিক কোম্পানি রাখার জন্য অ্যারে
  const [isFormOpen, setIsFormOpen] = useState(false); // ফর্ম ভিউ কন্ট্রোল করার জন্য
  const [editingCompany, setEditingCompany] = useState(null); // কোন কোম্পানি এডিট হচ্ছে তা ট্র্যাক করতে
  const [isLoading, setIsLoading] = useState(false); // সাবমিট লোডিং স্টেট
  ...
  //  ৩. প্রফেশনাল ImgBB ইমেজ আপলোড লজিক (নিরাপদ ও এরর হ্যান্ডেলিং সহ)
  const handleImageUpload = async (e) => {
    const file = e.target.files[0];
    if (!file) return;

    // ফাইল সাইজ ভ্যালিডেশন ( ৫ এমবি এর বেশি হলে আপলোড হবে না)
    ...
    try {
      ...
      if (data.success) {
        setLogoUrl(data.data.url); // সফল হলে ডাইনামিক URL স্টেটে বসে যাবে
      } else {
      ...
      setIsUploading(false); // আপলোড শেষে স্পিনার বন্ধ হবে
    }
  };

  // ৪. ফর্ম সাবমিট হ্যান্ডলার (নতুন অ্যাড এবং এডিট দুটোই হ্যান্ডেল করবে)
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isUploading) return; // ছবি আপলোড চলাকালীন যেন কেউ সাবমিট না করতে পারে

    setIsLoading(true);
    const formData = new FormData(e.currentTarget);
    const rawCompanyData = Object.fromEntries(formData);

    // ফাইনাল অবজেক্ট ডাটাবেজ ফ্রেন্ডলি
    const finalCompanyData = {
      ...rawCompanyData,
      logo: logoUrl || "https://placehold.co/150?text=No+Logo", // লোগো না দিলে প্লেসহোল্ডার ইমেজ বসবে
    };

    try {
      // ব্যাকএন্ডের ডামি টাইমআউট ( এখানে API কল হবে)
      await new Promise((resolve) => setTimeout(resolve, 1200));

      if (editingCompany) {
        // এডিট মুড: এক্সিস্টিং কোম্পানির তথ্য আপডেট (Data Retention)
        setCompanies(
          companies.map((c) =>
      ...
      // সফলভাবে সাবমিট শেষে ফর্ম বন্ধ এবং স্টেট রিসেট
      setIsFormOpen(false);
  ...
  // ৫. অ্যাকশন ট্রিগার ফাংশনসমূহ (Best Practice)
  const handleAddNewClick = () => {
    setEditingCompany(null);
    setLogoUrl(""); // নতুন কোম্পানির জন্য ইমেজ স্টেট
    setIsFormOpen(true);
  };

  const handleEditClick = (comp) => {
    setEditingCompany(comp);
    setLogoUrl(comp.logo); // এডিটের সময় আগের ইমেজটি ধরে রাখবে যাতে ডিলিট না হয়ে যায়
    setIsFormOpen(true);
  };
      ...
      <div className="p-4 md:p-8 bg-[#09090b] min-h-screen text-white">
        <div className="max-w-2xl mx-auto">
          ...
          <Form onSubmit={handleSubmit} className="space-y-6">
            {/*  ড্রপজোন স্টাইল প্রফেশনাল ইমেজ আপলোডার স্পিনার সহ */}
            <div className="flex flex-col gap-2 w-full">
              ...
              <div className="flex items-center gap-5 p-4 border border-zinc-800 bg-zinc-900/20 rounded-2xl w-full">
                {/* ইমেজ প্রিভিউ বক্স এবং ডার্ক লোডিং  */}
            ...
            <div className="flex justify-end gap-4 pt-6 border-t border-zinc-800/50">
              ...
              <Button
                type="submit"
                isLoading={isLoading}
                disabled={isUploading} // ছবি আপলোড চলাকালীন বাটন লক থাকবে
  ...
  // ৪. একাধিক কোম্পানি তালিকা বা ড্যাশবোর্ড ভিউ (প্লাস বাটন সহ)
  return (
    <div className="p-4 md:p-8 bg-[#09090b] min-h-screen text-white">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center pb-5 border-b border-zinc-800">
          ...
          {/* ➕ প্লাস বাটন যা দিয়ে রিক্রুটার আরও কোম্পানি যুক্ত হবে */}
          <Button
            onClick={handleAddNewClick}
